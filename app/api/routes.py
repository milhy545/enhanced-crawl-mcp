    ...
    try:
        ...
        update_job(
            job_id,
            {
                ...
                "failed": failed,
                "results": [r.model_dump() for r in crawl_responses],
                "updated_at": datetime.utcnow(),
            },
        )
        ...
        # If Playwright browsers are not installed, return 400 (bad request/environment)
        msg_lower = str(e).lower()
        is_playwright_env_issue = (
            "playwright" in msg_lower or "executable doesn't exist" in msg_lower
        )
        status_code = 400 if is_playwright_env_issue else 500
        raise HTTPException(
            status_code=status_code, detail={"error": "BatchCrawlError", "message": str(e)}
        ) from e
