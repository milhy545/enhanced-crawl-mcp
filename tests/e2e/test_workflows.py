...
class TestWorkflows:
    ...
    def test_complete_crawl_workflow(self, mock_crawler_class, client):
        ...
        response = client.post(
            "/crawl",
            json={"url": "https://example.com", "include_links": True, "include_images": True},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_batch_crawl_workflow(self, mock_crawler_class, client):
        ...
        # Create mock results
        mock_result1 = CrawlResult(url="https://example.com", markdown="# Example 1", success=True)
        mock_result2 = CrawlResult(url="https://example.org", markdown="# Example 2", success=True)
        ...
        response = client.post(
            "/crawl/batch", json={"urls": ["https://example.com", "https://example.org"]}
        )
        assert response.status_code == 200
        data = response.json()
        job_id = data["job_id"]
